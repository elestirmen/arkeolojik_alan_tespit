...
class TiiLIDARDatasetBinaryWithPreprocessingSchema(BaseDatasetSchema):
    data_dir = fields.String(
        missing="/", description="Dataset path on disk", example="./data/BigEarthNet/"
    )
    annotations_dir = fields.String(
        missing=None, description="Semantic masks path on disk", example="./data/masks",
    )
    object_class = fields.String(
        missing='AO', description="Object class", example="barrow",
    )
    DFM_quality = fields.String(
        missing='1,2,4', description="DFM quality", example="1,2,4",
    )
    visualisation_type = fields.String(
        missing='SLRM', description="Visualisation type", example="SLRM",
    )
    keep_empty_patches = fields.Bool(
        missing=False, description="Whether to keep patches with no objects of interest", example=False
    )
    object_class_band_id = fields.Integer(
        missing=None, description="The band where the label information is encoded within the segmentation mask for the given object class", example=1
    )
...
class ObjectDetectionCocoDatasetTIIDatasetSchema(ObjectDetectionCocoDatasetSchema):
    annotations_dir = fields.String(
        missing="/", description="BBox annotations directory path", example="./data/DIOR/"
    )
    object_class = fields.String(
        missing='AO', description="Object class", example="barrow",
    )
